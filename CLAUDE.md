...
### 5.7 価格の乱数は tick 番号の純粋関数にする

価格の基準となる乱数は、`rand.New(rand.NewSource(seed))` からの**逐次生成で実装してはならない**。
`(seed, tick番号)` を与えると値が返る純粋関数として実装する。

**逐次生成が禁止される理由:** Go の `math/rand` は途中位置からの再開ができないため、
セッション開始のたびに通貨生成時点から全 tick を引き直す必要が生じ、
運用日数に比例して計算量が増え続ける。純粋関数なら経過 tick 数から直接値を求められ、
セッション外にサーバーを起動し続ける必要もなくなる（scale-to-zero との相性）。

詳細・実装例は `docs/design.md` §2.7 を参照。
